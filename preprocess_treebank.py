...
from parser import parse_unimorph_features
import fasttext


config = get_config()["data"]
_DEFAULT_TREEBANKS_ROOT = config["ud_treebanks_root"]
...
fasttext_model = args.fasttext if args.fasttext else None
...
for f in os.listdir(treebank_path):
    if path.isfile(path.join(treebank_path, f)) and "-um-" in f and f.endswith(".conllu"):
        ...
        if args.bert:
            ...
            # TODO: Need to remove dependency here. Can use default fastText lib, which is already installed
            model = fasttext.load_model(fasttext_model)

            for sent_id, token_list in enumerate(tqdm(final_token_list)):
                total += 1
                for t in token_list:
                    t["embedding"] = model[t["form"]]
